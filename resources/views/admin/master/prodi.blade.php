...
<script>
    ...
    table = $('#grdDataGeografi').DataTable({
        ...
        ajax: {
            url: '{{ url("admin/master/prodi/all") }}',
        ...
        columns: [
            ...
            { data: "prodi_id", name: "prodi_id", orderable: false, 
                render: function(data, type, row, meta){
                    var elShow = '<a class="btn btn-sm btn-default" href="javascript: editData(\''+row.prodi_id+'\');"><i class="fa fa-eye"></i></a>';
    ...
    function editData(prodi_id){
        postData = new Object();
		postData.prodi_id = prodi_id;
		ajax({
			url : "{{ url('admin/master/prodi/show') }}", 
			postData : postData,
			success : function(ret){
                ...
                $('#prodi_name').val(data.prodi_name);
				$('#fakultas_id').val(data.fakultas_id).trigger('change');
	...
	$('#frmData').submit(function(){
		...
		$.ajax({
            ...
            data: formData,
			url : "{{ url('admin/master/prodi/save') }}",
            contentType: false,
            processData: false,
            cache: false,
...
<div class="row">
	<div class="row col-md-12">
		<div class="">
			<div class="col-md-12">
				<button id="btnAdd" class="btn btn-default">Tambah Data Prodi</button>
			</div>
		</div>
		<br><br>
		@if (Session::has('msg'))
		...
		<div class="modal-content">
			...
			<form class="form-horizontal" id="frmData" onSubmit="return false" method="post" enctype="multipart/form-data" action="#">
				<div class="modal-body">
					...
					<input type="text" name="prodi_id" id="prodi_id" class="hidden">

                    <div class="form-group">
                        <label class="col-sm-3 control-label" for="fakultas_id">Fakultas </label>
                        <div class="col-sm-9">
							<select name="fakultas_id" id="fakultas_id" class="select2" required>
                                <option value="" disabled selected hidden>Pilih Fakultas </option>
									@foreach ($master_fakultas as $key => $fakultas)
                                    	<option value="{{ $key }}"> {{ $fakultas }}</option>
									@endforeach     
                            </select>
                        </div>
                    </div>
					...
					<div class="form-group">
                        <label class="col-sm-3 control-label" for="prodi_name">Nama Program Studi</label>
                        <div class="col-sm-9">
							<input type="text" name="prodi_name" id="prodi_name" required class="form-control">
                        </div>
                    </div>
                </div>
